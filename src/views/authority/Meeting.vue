<template>
	<Row type="flex" justify="center" align="middle">
			...
			<Tabs :value="active" @on-click="changeTab">
					...
					<div id="mi-meeting-mine">
						<div class="mi-meeting-list">
							...
							<div class="mi-meeting-item" v-for="(meet, index) in meeting.mine" :key="index">
								<div class="mi-meeting-item-title">
									...
									<div class="share">
										<Tooltip content="修改会议" placement="top" style="margin-right: 8px;">
											<icon type="ios-create-outline" size="24" @click="setMeetingModal(meet.meetingId)" />
										</Tooltip>
										<Tooltip content="分享会议" placement="top" style="margin-right: 8px;">
											<icon type="ios-share-alt-outline" size="24" @click="setShareModal(meet.meetingId)" />
										</Tooltip>
										<Tooltip content="取消会议" placement="top">
											<icon type="ios-close-circle-outline" size="22" @click="setDeleteModal(meet.meetingId)" />
										</Tooltip>
									</div>
								</div>
								...
								<div class="mi-meeting-item-people" v-if="meet.type === 1">
									<span>参会人员：</span>
									<span class="name" v-for="(user, i) in meet.users" :key="i" v-html="user.realName"></span>
								</div>
							</div>
						...
						<div class="mi-pagination">
			                <span class="pagination-total">
			                    共 {{ total.mine }} 条记录 第 {{ pagination.mine.pageNum }} / {{ Math.ceil(total.mine / pagination.mine.pageSize) }} 页
			                </span>
			                <Page :total="total.mine" :current="pagination.mine.pageNum" :page-size="pagination.mine.pageSize" @on-change="setPaginationNum" @on-page-size-change="setPaginationSize" show-elevator show-sizer></Page>
			            </div>
					</div>
				</TabPane>
				<TabPane label="待参加会议" name="soon" id="soon">
					<div id="mi-meeting-soon">
						...
						<div class="mi-pagination">
			                <span class="pagination-total">
			                    共 {{ total.soon }} 条记录 第 {{ pagination.soon.pageNum }} / {{ Math.ceil(total.soon / pagination.soon.pageSize) }} 页
			                </span>
			                <Page :total="total.soon" :current="pagination.soon.pageNum" :page-size="pagination.soon.pageSize" @on-change="setPaginationNum" @on-page-size-change="setPaginationSize" show-elevator show-sizer></Page>
			            </div>
					</div>
				</TabPane>
				<TabPane label="历史会议" name="history" id="history">
					<div id="mi-meeting-history">
						...
						<div class="mi-pagination">
			                <span class="pagination-total">
			                    共 {{ total.history }} 条记录 第 {{ pagination.history.pageNum }} / {{ Math.ceil(total.history / pagination.history.pageSize) }} 页
			                </span>
			                <Page :total="total.history" :current="pagination.history.pageNum" :page-size="pagination.history.pageSize" @on-change="setPaginationNum" @on-page-size-change="setPaginationSize" show-elevator show-sizer></Page>
			            </div>
					</div>
				</TabPane>
			</Tabs>
		</Col>
		<Modal title="创建会议" class-name="mi-modal" width="540" v-model="modal" footer-hide>
			<Form :model="model" :rules="rules" :label-width="80" style="margin-top: 16px;" ref="form">
				<FormItem prop="subject" label="会议主题">
					<Input v-model="model.subject" maxlength="32" placeholder="请输入会议主题" />
				</FormItem>
				<FormItem prop="datetime" label="会议时间">
					<DatePicker type="date" v-model="model.date" :options="date" placeholder="请选择会议时间" @on-change="setDate" clearable :editable="false"></DatePicker>
					<TimePicker type="timerange" v-model="model.time" placeholder="请选择会议具体时间点" @on-change="setTime" :editable="false" style="margin-left: 16px;"></TimePicker>
				</FormItem>
				...
				<FormItem prop="people" label="参会人员" v-if="model.type === '1'">
					<div class="mi-modal-item-people">
						<div class="mi-modal-item-people-search">
							<Input type="text" v-model="name" suffix="ios-search" placeholder="搜索用户名" :max-length="64" />
						</div>
				...
				<FormItem style="margin-bottom: 0">
					...
					<Button @click="setMeetingModal()" style="margin-left: 16px;">取消</Button>
				</FormItem>
			</Form>
		</Modal>
		...
		<Modal class-name="mi-modal mi-modal-delete" width="300" v-model="confirm.modal" footer-hide>
...
<script>
    ...
    const AuthorityMeetingComponent = {
                ...
                if (Object.keys(value).length > 0) {
                    if (!value.date) {
                        ...
                        const stime = new Date(`${value.date} ${value.time[0]}`),
	                        etime = new Date(`${value.date} ${value.time[1]}`),
	                        dtime = etime - stime;
                        if (dtime <= 0) {
	    ...
	    methods: {
            logout() {
                this.$api.get('user-manager/logout', {}, (res) => {
		    ...
		    getUsers() {
                this.$api.get('/user-manager/user-tree', {
		    ...
		    getMeeting() {
                this.$api.get('/meeting-manager/meetings', {
                ...
                this.$refs.form.validate((valid) => {
                    if (valid) {
                        ...
                        if (this.model.id) {
                            ...
                            this.$api.post('/meeting-manager/meeting', params, (res) => {
                                this.loading = false;
	                            if (res['ret']['retCode'] === '0') {
		    ...
		    editMeeting(id) {
                if (!id || isNaN(id)) {
                    ...
                    this.$api.get(`/meeting-manager/meeting/${id}`, {}, (res) => {
                        if (res['ret']['retCode'] === '0') {
		    ...
		    updateMeeting(params) {
                ...
                this.$api.put(`meeting-manager/meeting/${this.model.id}`, params, (res) => {
                    this.loading = false;
                    if (res['ret']['retCode'] === '0') {
                        this.model.id = 0;
		    ...
		    deleteMeeting() {
                ...
                if (id && !isNaN(id)) {
                    this.$api.delete(`/meeting-manager/meeting/${id}`, {}, (res) => {
                        if (res['ret']['retCode'] === '0') {
                            this.setDeleteModal();
                ...
                for (let i in this.cates) {
                    if (this.cates.hasOwnProperty(i)) {
                        const id = `mi-meeting-${i}`,
	                        height = document.getElementById(id).clientHeight + 64,
...
<style lang="less">
	...
	.@{mi-modal} {
		...
		justify-content: center;
		.ivu-modal {top: 0;}
		.ivu-modal-body {padding: 16px 32px 24px 16px;}
		&-item {
			display: flex;
			align-items: center;
			justify-content: flex-start;
		...
		&-delete {
			.ivu-modal-body {padding: 16px 16px 24px;}
		}
		&-confirm {
			padding: 16px 0 0;
			display: flex;
