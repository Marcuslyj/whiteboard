...
<template>
  <div class="toolbar">
    <div class="left part" v-if="isSimple">
    ...
    <div class="center">
      <ul class="group draw-tool">
        ...
        <div class="menu pencil">
            ...
            <div class="tool-control row">
              <div  v-for="(pencilTool,index) in pencilToolArr" :Key="index" :class="{'item-wrapper':true,'active-item':activePencilTool===pencilTool.name}" >
                <span><i :class="['iconfont',pencilTool.icon]"></i></span>
              </div>
            </div>
            <div class="color-control row">
              <div v-for="(color,index) in pencilColorArr" :key="index" :class="{'item-wrapper':true,'active-item':activePencilColor===color}" >
            ...
            <div class="width-control row">
                ...
                <div v-for="(radius,index) in radiusArr" :key="index" :class="{'item-wrapper':true,'active-item':activePencilWidth===radius}">
                  <span class="cirlce" :style="{width:`${radius}vw`,height:`${radius}vw`,'border-radius':`${radius/2}vw`,backgroundColor:`${activePencilColor}`}" ></span>
                </div>
            </div>
          </div>
        </li>
        <li>
          <i class="iconfont icon-eraser"></i>
           <div class="menu eraser">
              <div class="row">
                <div  v-for="(eraserTool,index) in eraserToolArr" :Key="index" :class="{'item-wrapper':true,'active-item':activeEraserTool===eraserTool.name}" >
                  <span><i :class="['iconfont',eraserTool.icon]"></i></span>
                </div>
              </div>
              <div class="row width-control">
                <div class="width-level">{{activePencilWidth}}</div>
                <div v-for="(radius,index) in radiusArr" :key="index" :class="{'item-wrapper':true,'active-item':activeEraserWidth===radius}">
                  <span class="circle" :style="{width:`${radius}vw`,height:`${radius}vw`,'border-radius':`${radius/2}vw`}" ></span>
                </div>
              </div>
           </div>
        </li>
      ...
      <ul class="group bussiness-tool">
        <li><i class="iconfont icon-upload"></i></li>
